...
const ProductDisplay = ({ product }) => {
    ...
    // Function to calculate estimated delivery by the end of this week with a dummy pin code
    const getDeliveryTag = () => {
        const pinCode = '123456'; // Dummy pin code for demonstration
        const today = new Date();
        const endOfWeek = new Date(today);
        endOfWeek.setDate(endOfWeek.getDate() + (6 - endOfWeek.getDay())); // Calculate end of current week (assuming week starts on Sunday)

        if (product.deliveryDate) {
            const deliveryDate = new Date(product.deliveryDate);
            if (deliveryDate <= endOfWeek) {
                return `Estimated Delivery: By this week (${pinCode})`;
            } else {
                return `Estimated Delivery: Later (${pinCode})`;
            }
        } else {
    ...
    return (
        <div className="productdisplay">
            ...
            <div className="productdisplay-right">
                ...
                <p> Estimated Delivery Date: By this week </p>
            </div>
        </div>
    );
};
