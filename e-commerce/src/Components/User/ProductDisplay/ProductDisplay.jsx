...
const ProductDisplay = ({ product, image }) => {
    ...
    // Handle Pincode Check API Integration
    const handlePincodeCheck = async () => {
        ...
        try {
            const response = await fetch('http://localhost:5000/api/pincode/check', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
            ...
            if (response.ok) {
                if (data.available) {
                    setEstimatedDelivery(`Delivery available in ${data.estimatedDeliveryDays} days.`);
                } else {
                    setEstimatedDelivery('Pincode is not available for delivery.');
                }
            } else {
                setEstimatedDelivery('Error checking pincode availability.');
            }
        } catch (error) {
            console.error('Pincode check failed:', error);
            setEstimatedDelivery('Failed to check pincode availability.');
        } finally {
            ...
            <div className="productdisplay-right">
                ...
                <div className="productdisplay-pincode">
                    <input
                        ...
                        onChange={(e) => setPincode(e.target.value)}
                    />
                    <button onClick={handlePincodeCheck}>Check Delivery</button>
